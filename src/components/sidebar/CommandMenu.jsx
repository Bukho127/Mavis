...
import { Loading03Icon, Mic01Icon, ViewIcon, PlusSignCircleIcon } from '@hugeicons/core-free-icons'
...
const CommandMenu = ({ open, setOpen }) => {
  ...
  const handleSelectInterview = (interview) => {
    setOpen(false)
    navigate('/dashboard/feedback', { state: { selectedInterview: interview } })
  }
  ...
  const filteredInterviews = !normalizedQuery
    ...
    : interviews
        .filter((interview) => {
          ...
          return role.includes(normalizedQuery) || date.includes(normalizedQuery)
        })
        .slice(0, 8)
        ...
        <div>
          ...
          {!loading && !error && filteredInterviews.length > 0 && (
            <div className='mb-2 p-3'>
              ...
              {filteredInterviews.map((interview) => (
                ...
                >
                  <HugeiconsIcon icon={Mic01Icon} size={16} className='shrink-0 text-[#4A7FF8]' />
                  <div className='min-w-0 flex-1'>
                    <div className='flex items-center gap-2'>
                      <span className='font-medium text-stone-950'>{interview.role}</span>
          ...
          {!loading && !error && filteredInterviews.length === 0 && (
            <div className='p-3 text-sm text-stone-500'>
              No interview matches for <span className='font-medium text-stone-700'>"{value}"</span>
            </div>
          )}
